defmodule Solution do
  ...
  def sum_chunk([first, second, third]) do
    first = first |> String.replace("\n", "") |> String.graphemes() |> MapSet.new()
    second = second |> String.replace("\n", "") |> String.graphemes() |> MapSet.new()
    third = third |> String.replace("\n", "") |> String.graphemes() |> MapSet.new()
  ...
  def get_priority(letter, :first_attempt) do
  ...
  def get_priority(letter, :second_attempt) do
    case :binary.match("ABCDEFGHIJKLMNOPQRSTUVWXYZ", letter) do
      :nomatch -> nil
